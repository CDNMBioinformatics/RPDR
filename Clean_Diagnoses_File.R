...
process_diagnoses_set_range <- function(DF_to_fill = All_merged,
  ...
  Diagnoses <- data.table(fread(str_c(input_file_header, "Dia", input_file_ending)))
  if (!dir.exists(path_dia_abn)) {dir.create(path_dia_abn)}
  logdebug(str_c("Note: All Diagnoses abnormalites can be found at ", path_dia_abn))
  ...
  loginfo(str_c("Restricting Dia to specified date range... "))
  Date_Range_DF <- DF_to_fill %>% select(EMPI, min_dates, max_dates)
  Diagnoses <- left_join(Diagnoses, Date_Range_DF)
  Original_row_length <- nrow(Diagnoses)
